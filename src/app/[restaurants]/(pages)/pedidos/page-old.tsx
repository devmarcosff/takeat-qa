                ...
                <TabsContent key={tab} value={tab} className="pb-32">
                  {filteredBaskets.length > 0 ? (
                    filteredBaskets.map(({ basket, userChange }) => (
                      <CardComponent key={basket.id} basket={basket} userChange={userChange} restaurant={restaurant} />
                    ))
                  ) : (
...
const CardComponent = ({ basket, userChange, restaurant }: { basket: OrderBasketItem, userChange?: string, restaurant: string }) => {
  ...
  return (
    <div className="max-w-3xl rounded-2xl bg-white shadow-md overflow-hidden cursor-pointer my-4 border" onClick={() => localStorage.setItem(`@infoPedidosTakeat:${restaurant}`, JSON.stringify(basket))}>
